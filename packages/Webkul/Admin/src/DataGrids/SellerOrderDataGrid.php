...
class SellerOrderDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        $queryBuilder = DB::table('orders')
            ...
            ->leftJoin('products as products', function($leftJoin) {
                $leftJoin->on('products.id', '=', 'order_items.product_id')
                    ->where('products.seller_id', '!=', auth()->guard('admin')->id());
            })
            ->leftJoin('admins as admins', function($leftJoin) {
                $leftJoin->on('admins.id', '=', 'products.seller_id');
            })
            ->leftJoin('roles as roles', function($leftJoin) {
                $leftJoin->on('roles.id', '=', 'admins.role_id');
            })
            ->where('products.seller_id', '!=', auth()->guard('admin')->id())
            ->addSelect('admins.name as admin_name', 'admins.id as admin_id', 'roles.name as role_name', 'products.seller_id as seller_id')
