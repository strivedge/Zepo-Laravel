...
@section('page_title')
    {{__('admin::app.catalog.customer.edit-title') }}
@stop
    ...
    <form method="POST" action="{{ route('admin.catalog.customer.save') }}" @submit.prevent="onSubmit" id="price_data_form">
        ...
        <div class="page-content">
            <div class="form-container">
                ...
                <input type="hidden" name="data[]" id="customer_price_data">
                 ...
                 <div class="control-group" :class="[errors.has('product_id') ? 'has-error' : '']">
                    <label for="area_name" class="required">{{ __('admin::app.catalog.customer.product') }}</label>
                     <select id="product_id" name="product_id" data-vv-as="&quot;{{ __('admin::app.catalog.customer.product') }}&quot;" class="control product_id" aria-invalid="false" value="{{$product_id}}">
                        @foreach ($products as $ck=>$prod)
                            <option value="{{$prod->product_id}}" {{$prod->product_id == $product_id ? 'selected' : ''}}>{{$prod->product_name}} </option> 
                        @endforeach
                    </select>
                    <div>
                ...
                <div id="customer_price_div">
                    @if(count($data) > 0)
                        ...
                        <div class="customer_price_div" id="customer_price_div_{{$k}}">
                            <div class="control-group" :class="[errors.has('customer_id') ? 'has-error' : '']">
                                <label for="customer_id__{{$k}}" class="required">{{ __('admin::app.catalog.customer.customer') }}<span style="color:red;text-align: right;" class="remove_div" id='{{$k}}'>Remove</span></label>
                                 <select  data-id="customer_id_err_{{$k}}" id="customer_id_{{$k}}" name="customer_id[]" data-vv-as="&quot;{{ __('admin::app.catalog.customer.customer') }}&quot;"  value="{{$val->customer_id}}" class="control customer_id" aria-invalid="false">
                                    @foreach ($customers as $ck=>$cust)
                                        <option value="{{$cust->customer_id}}" {{$val->customer_id == $cust->customer_id ? 'selected' : ''}}>{{$cust->full_name}} </option> 
                                    @endforeach
                                </select>
                                <div>
                                    <span class="control-error" id="customer_id_err_{{$k}}" style="display: none">
                                       {{ __('admin::app.catalog.customer.required') }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="control-group" :class="[errors.has('price') ? 'has-error' : '']">
                                <label for="customer_price_{{$k}}" class="required">{{ __('admin::app.catalog.customer.price') }}</label>
                                 <input type="number" data-id="customer_price_{{$k}}" name="customer_price[]" value="{{$val->price}}" data-vv-as="&quot;{{__('offer::app.catalog.customer.price') }}&quot;" class="control customer_price" aria-required="true" aria-invalid="false"> 
                                <div>
                                    <span class="control-error" id="customer_price_{{$k}}" style="display: none">
                                        {{ __('admin::app.catalog.customer.required') }}
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="customer_price_div" id="customer_price_div_1">
                            <div class="control-group" :class="[errors.has('customer_id') ? 'has-error' : '']">
                                <label for="area_name" class="required">{{ __('admin::app.catalog.customer.customer') }}<span style="color:red;text-align: right;" class="remove_div" id='1'>Remove</span></label>
        ...
        $(document).ready(function () {
           ...
           //console.log('customer',customer)
            ...
            $(document.body).on('click','#save_customer_price', function(e) { 
                console.log('document save_customer_price')
